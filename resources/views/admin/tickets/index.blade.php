    ...
    <form class="filter-bar d-flex flex-wrap align-items-end gap-2">
        <div>
            ...
            <div class="input-group">
                ...
                <input name="search" class="form-control" placeholder="Ticket, subject, customer" value="{{ request('search') }}">
            </div>
        </div>
        <div>
        ...
        <a href="{{ route('admin.support-tickets.index') }}" class="btn btn-outline-secondary" title="Reset"><i class="fa-solid fa-rotate-left"></i></a>
    </form>
</div>
        ...
        <table class="table align-middle mb-0">
            ...
            @forelse($tickets as $ticket)
                <tr>
                    ...
                    <td>{{ $ticket->user?->name }}<div class="small text-muted">{{ $ticket->user?->email }}</div></td>
                    ...
                    <td class="text-end">
                        <div class="table-actions">
                            <a href="{{ route('admin.support-tickets.show', $ticket) }}" class="btn btn-sm btn-outline-secondary" title="View"><i class="fa-solid fa-eye"></i></a>
                        </div>
                    </td>
                </tr>
            @empty
                <tr><td colspan="6" class="text-center text-muted py-5"><i class="fa-solid fa-inbox fa-2x d-block mb-2"></i>No tickets found.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
